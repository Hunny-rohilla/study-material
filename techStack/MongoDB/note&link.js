//? YouTube Link: https://www.youtube.com/watch?v=rU9ZODw5yvU&t=81s
...
//# ******************** Key Concepts: **************************
//* Documents:
// The fundamental unit of data storage in MongoDB. A document is a self-contained unit resembling a JSON object, with key-value pairs holding data.
//*Collections:
// Groups of logically related documents. They are akin to tables in relational databases, but without a predefined schema.
//* Databases:
// Collections are organized within databases, providing a way to categorize and manage large amounts of data.
//* Indexes:
// Similar to indexes in relational databases, MongoDB indexes improve query performance by creating ordered listings of specific fields or combinations of fields.
//* Schema Flexibility:
// MongoDB allows for documents within a collection to have different structures, unlike relational databases with a rigid schema.
//* Relationships:
// While MongoDB doesn't enforce relationships like relational databases, it offers embedded documents and references (DBRef) to model relationships within documents or across collections.
//* Replication:
// Enables data redundancy and high availability by maintaining copies of the database on multiple servers.
//* Sharding:
// Horizontally scales data storage by distributing collections across multiple shards (partitions) on different servers, handling large datasets efficiently.
...
// ### Important Concepts:
//* 1. **Document:**
// - A document in MongoDB is a set of key-value pairs, similar to JSON objects. It is the basic unit of data storage.
```
     {
...
//* 2. **Collection:**
// - A collection is a grouping of MongoDB documents. It is equivalent to an RDBMS table.
db.users.insertOne({
  name: "Alice",
  age: 25,
});
...
//* 3. **Indexing:**
// - Indexes in MongoDB improve the efficiency of read operations. They provide a quick way to locate documents based on specific fields.
db.users.createIndex({ name: 1 });

//* 4. **Querying:**
// - MongoDB supports rich query operations, including filtering, sorting, and projection, to retrieve specific documents from a collection.
db.users.find({ age: { $gt: 25 } });

//* 5. **Aggregation:**
// - MongoDB's aggregation framework allows the processing of data records to produce aggregated results.
